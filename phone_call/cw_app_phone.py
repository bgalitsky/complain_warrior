...
def main():
    ...
    with right:
        ...
        if not drafts:
            ...
            # After sending at least one email in this thread, enable the phone call action
            st.divider()
            st.markdown("### Phone call")
            can_call = st.session_state.manager.can_make_call(cid, tid)
            vendor_hint = st.text_input(
                "Vendor hint (optional)",
                value=(ts.last_phone_reply or {}).get("vendor_hint", ""),
                disabled=not can_call,
                help="Optional: helps GPT pick correct customer support phone numbers.",
            )

            if not can_call:
                st.info("Send an email in this thread first — then the 'Make a call' button will appear here.")
            else:
                if st.button("Make a call now (auto-route)"):
                    try:
                        _log("Starting phone call via Twilio (auto-route)…")
                        reply = st.session_state.manager.make_support_call(cid, tid, vendor_hint=vendor_hint or None, timeout_s=300)
                        st.success("Call completed. Reply captured.")
                        # refresh state
                        cs = st.session_state.manager.get_complaint(cid)
                        ts = cs.threads.get(tid)
                    except Exception as e:
            ...
            # Show last captured phone reply
            if ts.last_phone_reply and (ts.last_phone_reply.get("text") or "").strip():
                st.text(f"Call time: {ts.last_phone_reply.get('ts','')}")
                st.text_area("Customer support reply (phone)", value=ts.last_phone_reply.get("text", ""), height=140)
            else:
                st.caption("(no phone reply captured yet)")
